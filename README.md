...
* **Explanation:** Established secure baseline via Note 20 Ultra. Initialized Git version control and configured SSH key-based authentication. Documented initial Linux navigation and file management via discovery.txt.

### 🛠️ T1-M1-S02: Command Line Operations
* [Evidence: Commit f244294](https://github.com/CK-Bachoo/IF-Cyber-Portfolio/commit/f244294)
* **Explanation:** CLI proficiency. Demonstrated mastery of directory traversal and core Linux I/O operations required for headless systems administration.
...
* **Explanation:** Advanced log interrogation to identify Indicators of Compromise (IoCs). Forensic IP Extraction: 10.99.88.77.
...
* **Explanation:** Connectivity verification. Leveraged ip addr and ping in Termux to blueprint subnetting architectures (Target: 192.168.1.200).
...
* **Explanation:** Protocol parsing via grep, awk, and sed in Termux. Isolated network protocol anomalies from raw data (Google IP: 142.250.217.14).

### 🛰️ T1-M2.TLAB: Operation Blackout (Linux Hardening)
* [Evidence: harden.sh](https://github.com/CK-Bachoo/IF-Cyber-Portfolio/blob/main/harden.sh)
* **Explanation:** Initialization of Module 2 Capstone. Leveraging **Google Cloud Shell via Phone Chrome Browser** for high-compute security assessments and advanced automation workflows using the Note 20 Ultra.
...
| **Evidence & Data** | Automated Script Execution | 1. [Evidence 1: port_check.py](https://github.com/CK-Bachoo/IF-Cyber-Portfolio/blob/main/port_check.py) (Socket Logic) <br> 2. [Evidence 2: s07reflection.md](https://github.com/CK-Bachoo/IF-Cyber-Portfolio/blob/main/s07reflection.md) |
...
**🛡️ S08 Technical Analysis:** Forensic task performed by capturing a live memory snapshot via `ps aux`. The system successfully extracted the target IPs (10.0.0.55 and 172.16.0.5) from active system noise by isolating PID 3845.
...
**🛡️ S09 Technical Analysis:** Android security model (Samsung Knox) blocked `sudo` provisioning. A custom Python auditor parsed JSON threat seeds manually to prove detection accuracy against threat signatures while the OS remained secured.
