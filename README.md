...
* [Evidence: Commit History](https://github.com/CK-Bachoo/IF-Cyber-Portfolio/commit/3e839e5)
...
* [Evidence: Commit History](https://github.com/CK-Bachoo/IF-Cyber-Portfolio/blob/main/network_audit.txt)
...
* **Explanation:** Network layer analysis. Leveraged Cisco Networking Academy resources to map the OSI model and TCP/IP stack. Conducted packet analysis foundations in preparation for advanced Wireshark interrogation (Ping 8.8.8.8: 0% loss).
...
* **Explanation:** Advanced log interrogation to identify Indicators of Compromise (IoCs). Detected unauthorized access attempts and proposed technical remediation (IP Forensic: 10.99.88.77).
...
* **Explanation:** Connectivity verification. Leveraged `ip addr`, `ping`, and `ipcalc` to blueprint subnetting architectures and ensure routing integrity (Target: 192.168.1.200).
...
* **Explanation:** Protocol parsing via `grep`, `awk`, and `sed`. Utilized Wireshark-based data captures to isolate network protocol anomalies (Google IP: 142.250.217.14).
