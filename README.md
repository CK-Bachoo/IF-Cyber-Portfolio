...
* [Evidence: discovery.txt](https://github.com/CK-Bachoo/IF-Cyber-Portfolio/blob/main/discovery.txt)
* **Explanation:** Established secure baseline via Note 20 Ultra. Documented initial Linux directory traversal and system navigation (Artifact: `discovery.txt`).
...
* **Explanation:** CLI proficiency. Demonstrated mastery of core Linux I/O operations required for headless systems administration.

### 🛠️ T1-M1-S03: Network Foundations (Cisco Academy)
* [Evidence: network_audit.txt](https://github.com/CK-Bachoo/IF-Cyber-Portfolio/blob/main/network_audit.txt)
* **Explanation:** Network layer analysis. Leveraged Cisco Academy resources to map the OSI model. Conducted packet analysis (Ping 8.8.8.8: 0% loss).
...
* **Explanation:** Advanced log interrogation to identify Indicators of Compromise (IoCs). Forensic IP: 10.99.88.77.
...
* **Explanation:** Infrastructure-as-Code deployment strictly via Google Cloud Shell and Termux.
...
* **Explanation:** Connectivity verification. Blueprinting architectures for routing integrity (Target: 192.168.1.200).
...
* **Explanation:** Protocol parsing via `grep`, `awk`, and `sed`. Captured anomalies (Google IP: 142.250.217.14).

---

## 🛰️ T1-M2.TLAB: Operation Blackout (Linux Hardening)
* [Evidence: harden.sh](https://github.com/CK-Bachoo/IF-Cyber-Portfolio/blob/main/harden.sh)
* **Explanation:** Initialization of Module 2 Capstone. Leveraging GitHub Codespaces for high-compute security assessments.

---

## 🐍 T1-M1.S07: THE AUTOMATION FORGE (Network Reconnaissance)

| Category | Desktop/Laptop User (Standard) | Android Cyber Workbench (Note 20 Ultra) |
| :--- | :--- | :--- |
...
**🛡️ S07 Technical Analysis:** Python-based socket logic executed within ARM64 Termux. Demonstrated the ability to handle low-level connection handshakes without root permissions.

---

## 🛡️ T1-M1.S08: REFINED PAPER TRAIL (Forensic Audit Comparison)

| Category | Desktop User (Standard) | Android Cyber Workbench (Note 20 Ultra) |
| :--- | :--- | :--- |
| **Source Data** | Static `auth_audit.log` file. | Live `ps aux` Memory Snapshot. |
| **Evidence & Data** | IPs: 10.0.0.55 / 172.16.0.5 | 1. [Evidence 1: brute_report.txt](https://github.com/CK-Bachoo/IF-Cyber-Portfolio/blob/main/brute_report.txt) <br> 2. [Evidence 2: monitoring_log.txt](https://github.com/CK-Bachoo/IF-Cyber-Portfolio/blob/main/monitoring_log.txt) |

**🛡️ S08 Technical Analysis:** Capturing live memory via `ps aux` to isolate PID 3845. Successfully extracted forensic IPs directly from active system noise (Timestamp: Tue Mar 24 2026).
...
| **Evidence & Data** | {"severity": "High"} (Automatic) | 1. [Evidence 1: audit_brief.txt](https://github.com/CK-Bachoo/IF-Cyber-Portfolio/blob/main/lab_prep/audit_brief.txt) <br> 2. [Evidence 2: system_auditor.py](https://github.com/CK-Bachoo/IF-Cyber-Portfolio/blob/main/system_auditor.py) |
| **Audit Result** | Malware Detected via Root Access. | [Result: [-] Clean / System Secured] |

**🛡️ S09 Technical Analysis:** Android security model blocked `sudo` provisioning. A custom Python auditor parsed JSON threat seeds manually to prove detection script accuracy against known threat signatures.
